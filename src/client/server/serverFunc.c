...
void sendListToUser(User *u) {
    ...
    if(u->acc) {
        ...
        sendMsgToUser(u, makeFailMsg(MSG_CONNECT, "Vous devez être connecté pour effectuer cette commande"));
    }
}
...
static void changeFileUserError(AccountFileError er, User *u) {
    ...
    switch(er) {
        case AADDFILE_NOERROR :
            ...
            sendMsgToUser(u, makeSuccessMsg(MSG_ADDFILE, "Fichier correctement ajouté à la liste d'attente de modification, attente du fichier"));
            break;

        case AADDFILE_NOFILE :
            HERROR_WRITE(HERROR_WARNING, "[Demande Modif. fich.] Impossible de lire la liste de fichier (ip '%s' | login '%s')", u->ip, u->acc->login);
            sendMsgToUser(u, makeFailMsg(MSG_ADDFILE, "Erreur sur la liste des fichiers à modifier"));
            break;
        ...
        case AADDFILE_QUOTA :
            ...
            sendMsgToUser(u, makeFailMsg(MSG_ADDFILE, "Quota de fichier dépassé, impossible de modifier le fichier"));
            break;

        case AADDFILE_EXIST :
            HERROR_WRITE(HERROR_WARNING, "[Demande Modif. fich.] Le fichier n'existe pas (ip '%s' | login '%s')", u->ip, u->acc->login);
            sendMsgToUser(u, makeFailMsg(MSG_ADDFILE, "Le fichiers à modifier n'existe pas. Impossible de le modifier"));
            break;
        ...
        case AADDFILE_LOCKED :
            ...
            sendMsgToUser(u, makeFailMsg(MSG_ADDFILE, " Le fichier est en cours d'opération par un autre utilisateur. Impossible de le modifier"));
            break;
...
static void deleteFileUserError(AccountFileError er, User *u) {
    ...
    switch(er) {
        case AADDFILE_NOERROR :
            ...
            sendMsgToUser(u, makeSuccessMsg(MSG_ADDFILE, "Fichier correctement supprimé"));
            break;

        case AADDFILE_NOFILE :
            HERROR_WRITE(HERROR_WARNING, "[Demande Supp. fich.] Impossible de lire la liste de fichier (ip '%s' | login '%s')", u->ip, u->acc->login);
            sendMsgToUser(u, makeFailMsg(MSG_ADDFILE, "Erreur sur la liste des fichiers à supprimer"));
            break;

        case AADDFILE_EXIST :
            HERROR_WRITE(HERROR_WARNING, "[Demande Supp. fich.] Le fichier n'existe pas (ip '%s' | login '%s')", u->ip, u->acc->login);
            sendMsgToUser(u, makeFailMsg(MSG_ADDFILE, "Le fichiers à supprimer n'existe pas. Impossible de le supprimer"));
            break;
        ...
        case AADDFILE_LOCKED :
            ...
            sendMsgToUser(u, makeFailMsg(MSG_ADDFILE, " Le fichier est en cours d'opération par un autre utilisateur. Impossible de le supprimer"));
            break;
